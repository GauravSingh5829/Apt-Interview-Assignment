...
Run the complete real-time CDC system locally with a single command:

```bash
# Start both Next.js frontend and PostgreSQL CDC engine concurrently
npm run dev:all

# Start with database reset to 0 orders
npm run dev:reset
```
...
|---|---|---|
| **dev:all** | `npm run dev:all` | Start Next.js frontend and PostgreSQL CDC simulator concurrently |
| **dev:reset** | `npm run dev:reset` | Start frontend and CDC engine with database reset to 0 orders |
| **db:reset** | `npm run db:reset` | Truncate database table back to 0 orders |
